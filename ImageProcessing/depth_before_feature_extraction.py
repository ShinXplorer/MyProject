...
def _orientation_matrix(frame: str) -> np.ndarray:
    ...
    if f == "z_up":
        # Camera coords: Xc right, Yc down, Zc forward
        # z_up requires Zw = +up. To keep a valid rotation (no reflection), pick Y-forward sign.
        # "+Y" (common robotics/DCC) : Xw =  Xc, Yw =  Zc, Zw = -Yc
        # "-Y" (Blender front view)  : Xw =  Xc, Yw = -Zc, Zw = -Yc
...
def save_point_cloud_from_disparity_masked(rect_left_bgr: np.ndarray,
    ...
    """
    Same as save_point_cloud_from_disparity, but ANDs with a 0/255 segmentation mask.
    """
...
def main() -> None:
    ...
    # (Optional) also see the rectified-left with mask applied
...
# 7b) Also write a **masked** PLY (background removed)
    ply_out_masked = os.path.join(OUTPUT_DIR, f"{base}_pcd_MASKED.ply")
    save_point_cloud_from_disparity_masked(rectL, disp, Q, ply_out_masked,
                                        fg_mask,
