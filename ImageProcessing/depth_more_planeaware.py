...
from new_feature_extract import (
    ...
    match_orb_between_rectified_pairs
)
...
PLANE_MIN_SAMPLES = 50
PLANE_RESID_THRESH = 1.0   # stricter acceptance (px)
PLANE_BLEND = 0.90        # blend less to preserve corners
ANCHOR_WEIGHT = 600.0
EDGE_PROTECT_RADIUS = 6   # pixels around edges/corners to protect from blending
...
def subpixel_refine_simple(disp: np.ndarray) -> np.ndarray:
    """Simple subpixel refinement: quadratic fit across ±1 integer disparities using local neighborhood.
    This is a lightweight heuristic to reduce staircasing.
    """
    h, w = disp.shape
    dsub = disp.copy().astype(np.float32)
    # For each pixel with integer disparity and valid neighbors, do a 1D quadratic fit along x (epipolar)
    for y in range(1, h-1):
        ...
        for x in idx:
            d0 = row[x]
            # check neighbors within row
            if x-1 < 0 or x+1 >= w: continue
            d1 = row[x-1]; d2 = row[x+1]
            if d1 <= 0 or d2 <= 0: continue
            # quadratic fit: f(-1)=d1, f(0)=d0, f(1)=d2 -> vertex shift
            denom = (d1 - 2*d0 + d2)
            if abs(denom) < 1e-6: continue
            shift = 0.5 * (d1 - d2) / denom
            if abs(shift) > 1.0: continue
            dsub[y, x] = d0 + shift
    return dsub
...
def extract_anchors_from_keypoints(rectL, rectR, keypoints_arr, min_disp, num_disp, template_thresh=0.6):
    ...
    h, w = gL.shape
    total_kp = len(keypoints_arr)
    kp_count = 0
    for row in keypoints_arr:
        x = int(round(float(row[0]))); y = int(round(float(row[1])))
        kp_count += 1
        if kp_count % 200 == 0:
            print(f"[anchors] processed {kp_count}/{total_kp} keypoints...")
...
def extract_anchors_orb_fallback(rectL, rectR, fg_mask, num_disp, epipolar_tol=2.0, ratio=0.75):
    """Fallback ORB descriptor matching to get anchors when precomputed keypoints yield none."""
    good, kpsL, kpsR, vis = match_orb_between_rectified_pairs(rectL, rectR, fg_mask, n_features=N_FEATURES, ratio=ratio, epipolar_tol=epipolar_tol)
    anchors = []
    if not good or not kpsL or not kpsR:
        return anchors
    for m in good:
        i = m.queryIdx; j = m.trainIdx
        xL, yL = map(int, map(round, kpsL[i].pt)); xR, yR = map(int, map(round, kpsR[j].pt))
        disp = xL - xR
        if disp <= 0 or disp > num_disp:
            continue
        anchors.append((xL, yL, float(disp)))
    return anchors


def fit_plane_irls(xs, ys, ds, anchors=None, max_iter=6, huber_delta=1.0, anchor_weight=ANCHOR_WEIGHT):
    """Robust IRLS plane fit using Huber weighting. Returns (a,b,c,median_res) or None."""
    if len(xs) < 3:
        return None
    A = np.column_stack([xs, ys, np.ones_like(xs)])
    b = ds.astype(np.float32)
    w = np.ones_like(b, dtype=np.float32)
    # incorporate anchors as extra rows with high initial weight
    if anchors:
        akp = np.array([[a[0], a[1], 1.0] for a in anchors], dtype=np.float32)
        ad = np.array([a[2] for a in anchors], dtype=np.float32)
        A = np.vstack([A, akp])
        b = np.concatenate([b, ad])
        w = np.concatenate([w, np.ones_like(ad, dtype=np.float32) * anchor_weight])
    ...
    for _ in range(max_iter):
        try:
            ...
            try:
                ...
                return None
        # compute residuals for original data length
        pred = (A @ x)[:len(ds)]
        res = b[:len(ds)] - pred
        absr = np.abs(res)
        # Huber weights
        ...
        for i in range(len(absr)):
            if absr[i] <= huber_delta:
                w[i] = 1.0
            else:
                w[i] = huber_delta / absr[i]
        # anchors weights stay high (tail of w)
        if anchors:
            w[len(ds):] = anchor_weight
        # small stabilization
        if np.allclose(w, w_old, atol=1e-3):
            break

    med_res = float(np.median(np.abs(res)))
    return float(x[0]), float(x[1]), float(x[2]), med_res


def refine_disparity_grid(disp, rectL, fg_mask, anchors, debug_dir=None):
    h, w = disp.shape
    out = disp.copy()
    # adaptive region size based on image area
    region = max(24, int(np.sqrt(0.5 * h * w / 100)))
    residual_map = np.full_like(disp, np.nan, dtype=np.float32)

    # build protection masks (edges & corners)
    gray = cv2.cvtColor(rectL, cv2.COLOR_BGR2GRAY)
    edges = cv2.Canny(gray, 50, 150)
    corners = np.zeros_like(edges)
    try:
        cpts = cv2.goodFeaturesToTrack(gray, maxCorners=800, qualityLevel=0.01, minDistance=4)
        if cpts is not None:
            for p in cpts:
                x, y = int(round(p[0][0])), int(round(p[0][1]))
                cv2.circle(corners, (x, y), 1, 255, -1)
    except Exception:
        pass
    protect = cv2.dilate(((edges > 0) | (corners > 0)).astype(np.uint8), cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (EDGE_PROTECT_RADIUS*2+1, EDGE_PROTECT_RADIUS*2+1)))

    total_regions = ((h + region - 1) // region) * ((w + region - 1) // region)
    reg_idx = 0
    for y0 in range(0, h, region):
        for x0 in range(0, w, region):
            reg_idx += 1
            if reg_idx % 50 == 0:
                print(f"[refine] regions processed {reg_idx}/{total_regions}...")
            ...
            res = fit_plane_irls(xs.astype(np.float32), ys.astype(np.float32), ds.astype(np.float32), anchors=reg_anchors if reg_anchors else None)
            if res is None:
                continue
            a, b, c, med_res = res
            ...
            if med_res > PLANE_RESID_THRESH:
                continue
            # apply blended plane values but protect edges/corners
            for (yy, xx), pval in zip(zip(ys, xs), (a * xs + b * ys + c)):
                if protect[yy, xx]:
                    # skip blending at protected pixels
                    continue
    ...
    if debug_dir:
        try:
            ...
            cv2.imwrite(os.path.join(debug_dir, "protect_mask.png"), (protect*255).astype(np.uint8))
        except Exception:
            pass
    return out
...
def main() -> None:
    ...
    # 3.5) Edge-preserving refinement (joint bilateral on refined disparity)
    if hasattr(cv2, "ximgproc"):
        guide32 = cv2.cvtColor(rectL, cv2.COLOR_BGR2GRAY).astype(np.float32)
        disp_mm = (use_disp * 1000.0).astype(np.float32)
        d = 9
        sigmaColor = 8.0
        sigmaSpace = 9.0
        disp_mm_f = cv2.ximgproc.jointBilateralFilter(guide32, disp_mm, d, sigmaColor, sigmaSpace)
        use_disp = disp_mm_f / 1000.0
    else:
        print("[warn] cv2.ximgproc not available — jointBilateralFilter skipped")

    # 4) Depth maps
    depth_mm_from_Q  = disparity_to_depth_Q(use_disp, Q)
    depth_m_from_fxB = disparity_to_depth_fxB(use_disp, P1, P2)

    # 5) Visualization
    disp_u8, disp_col = disp_to_vis(use_disp)
