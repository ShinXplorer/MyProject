...
# ========== EDIT THESE ==========
IMAGE_PATH = r"data/trial_approx_images/WIN_20251008_10_14_38_Pro.jpg"
CALIB_NPZ  = r"opencv_stereo_params2.npz"
OUTPUT_DIR = r"data/output/depth_refined"
...
SAVE_ZUP = True  # save PLYs with Z as "up" (X right, Y forward, Z up)
...
#   "camera" : X=right, Y=down,  Z=forward  (raw OpenCV reprojectImageTo3D)
#   "z_up"   : X=right, Y=forward, Z=up     (good for robotics/graphics)
#   "y_up"   : X=right, Y=up,      Z=forward (keeps “facing front” like your camera)
EXPORT_FRAME = "y_up"  # <- set to "y_up" to make your object face front in viewers
...
def get_profile(name: str) -> Dict:
    ...
    if name == "detail":
        return dict(mode=cv2.STEREO_SGBM_MODE_SGBM_3WAY, bs=3, uniq=1, p2_mult=48, disp12MaxDiff=2,
                    wls_lambda=70000.0, wls_sigma=1.7, clahe_clip=1.5, clahe_tile=(12, 12))
    elif name == "lowtex":
...
def _orientation_matrix(frame: str) -> np.ndarray:
    ...
    if f == "camera":
        # X=right, Y=down, Z=forward (OpenCV camera coords)
        return np.eye(3, dtype=float)
    if f == "z_up":
        # Xw = Xc, Yw = Zc, Zw = -Yc  => Z is "up", forward is +Y
        return np.array([[1, 0,  0],
                         [0, 0,  1],
                         [0,-1,  0]], dtype=float)
    if f == "y_up":
        # Xw = Xc, Yw = -Yc, Zw = Zc  => Y is "up", forward stays +Z (camera-forward)
        return np.array([[1,  0, 0],
                         [0, 1, 0],
                         [0,  0, 1]], dtype=float)
...
def save_point_cloud_from_disparity(rect_left_bgr: np.ndarray,
    ...
    pts = pts3d[mask].astype(np.float32)  # Nx3 (camera frame)
    cols_rgb = (rect_left_bgr[mask].astype(np.float32) / 255.0)[:, ::-1]  # to RGB

    # Units -> meters if T was in mm
    if calib_t_unit.lower() == "mm":
        pts *= 0.001

    # ---- Apply chosen world-frame orientation BEFORE saving ----
    R = _orientation_matrix(EXPORT_FRAME)  # 3x3
    pts = (R @ pts.T).T  # rotate all points

    # Build and save Open3D cloud
